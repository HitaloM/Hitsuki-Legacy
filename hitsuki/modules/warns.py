...
def warn(user: User, chat: Chat, reason: str, message: Message, warner: User = None, conn=False) -> str:
    ...
    if num_warns >= limit:
        ...
        if not soft_warn:
            if not warn_mode:
                ...
                reply = tl(chat.id, "{} peringatan, {} telah ditendang!").format(limit,
                ...
                reply = tl(chat.id, "{} peringatan, {} telah diblokir!").format(limit,
                                                                                mention_html(user.id, user.first_name))
            elif warn_mode == 3:
                message.bot.restrict_chat_member(chat.id, user.id, can_send_messages=False)
                reply = tl(chat.id, "{} peringatan, {} telah dibisukan!").format(limit,
                                                                                 mention_html(user.id, user.first_name))
        else:
            chat.kick_member(user.id)
            reply = tl(chat.id, "{} peringatan, {} telah diblokir!").format(limit,
        ...
        if num_warns + 1 == limit:
            ...
            reply = tl(chat.id,
                       "{} punya {}/{} peringatan... Jika anda di peringati lagi maka kamu akan di {}!").format(
    ...
    try:
        if conn:
            ...
            send_message_raw(chat.id, reply, reply_to_message_id=message.message_id, reply_markup=keyboard,
        ...
        if excp.message == "Reply message not found":
            ...
            if conn:
                ...
                try:
                    message.bot.sendMessage(chat.id, reply, reply_to_message_id=message.message_id,
                                            reply_markup=keyboard, parse_mode=ParseMode.HTML, quote=False)
                except BadRequest:
                    message.bot.sendMessage(chat.id, reply, reply_markup=keyboard, parse_mode=ParseMode.HTML,
                                            quote=False)
...
def button(update, context):
    ...
    if match:
        ...
        if res:
            ...
            update.effective_message.edit_text(
                tl(update.effective_message, "Pengguna sudah tidak memiliki peringatan.").format(
                    mention_html(user.id, user.first_name)),
                parse_mode=ParseMode.HTML)
...
def warn_user(update, context):
    ...
    if check.status == 'member' or check['can_restrict_members'] is False:
        if conn:
            text = tl(update.effective_message,
                      "Saya tidak bisa membatasi orang di {}! Pastikan saya sudah menjadi admin.").format(chat_name)
        else:
            text = tl(update.effective_message,
    ...
    if user_id:
        if conn:
            ...
            send_message(update.effective_message,
                         tl(update.effective_message, "Saya sudah memperingatinya pada grup *{}*").format(chat_name),
                         parse_mode="markdown")
            return warning
        else:
...
def reset_warns(update, context):
    ...
    if user_id and user_id != "error":
        ...
        if conn:
            send_message(update.effective_message,
                         tl(update.effective_message, "Peringatan telah disetel ulang pada *{}*!").format(chat_name),
                         parse_mode="markdown")
        else:
...
def warns(update, context):
    ...
    if result and result[0] != 0:
        ...
        if reasons:
            if conn:
                text = tl(update.effective_message,
                          "Pengguna ini memiliki {}/{} peringatan pada *{}*, untuk alasan berikut:").format(num_warns,
                                                                                                            limit,
                                                                                                            chat_name)
            else:
                text = tl(update.effective_message,
                          "Pengguna ini memiliki {}/{} peringatan, untuk alasan berikut:").format(num_warns, limit)
            ...
            if conn:
                send_message(update.effective_message,
                             tl(update.effective_message,
                                "Pengguna ini memiliki {}/{} peringatan pada *{}*, tetapi tidak ada alasan untuk itu.").format(
                                 num_warns, limit, chat_name), parse_mode="markdown")
            else:
                send_message(update.effective_message,
                             tl(update.effective_message,
                                "Pengguna ini memiliki {}/{} peringatan, tetapi tidak ada alasan untuk itu.").format(
                                 num_warns, limit))
    else:
        if conn:
            send_message(update.effective_message, tl(update.effective_message,
                                                      "Pengguna ini belum mendapatkan peringatan apa pun pada *{}*!").format(
                chat_name), parse_mode="markdown")
        else:
            send_message(update.effective_message,
                         tl(update.effective_message, "Pengguna ini belum mendapatkan peringatan apa pun!"))
...
def add_warn_filter(update, context):
    ...
    if conn:
        text = tl(update.effective_message, "Peringatkan handler yang ditambahkan untuk '{}' pada *{}*!").format(
            keyword, chat_name)
    else:
...
def remove_warn_filter(update, context):
    ...
    if nowarn == 0:
        if conn:
            text = tl(update.effective_message,
                      "Ya, saya akan berhenti memperingatkan orang-orang untuk {} pada *{}*.").format(success,
                                                                                                      chat_name)
        else:
            text = tl(update.effective_message, "Ya, saya akan berhenti memperingatkan orang-orang untuk {}.").format(
                success)
        send_message(update.effective_message, text, parse_mode=ParseMode.MARKDOWN)
        raise DispatcherHandlerStop
    elif inwarn == 0:
        if conn:
            text = tl(update.effective_message, "Gagal menghapus filter warn untuk {} pada *{}*.").format(fail,
                                                                                                          chat_name)
        else:
        ...
        if conn:
            text = tl(update.effective_message,
                ...
                success, fail, chat_name)
        else:
            text = tl(update.effective_message,
                      "Ya, saya akan berhenti memperingatkan orang-orang untuk {}.\nDan gagal menghapus filter warn untuk {}.").format(
                success, fail)
        send_message(update.effective_message, text, parse_mode=ParseMode.MARKDOWN)
        raise DispatcherHandlerStop
...
def list_warn_filters(update, context):
    ...
    if conn:
        filter_list = filter_list.replace(tl(update.effective_message, 'obrolan ini'),
    ...
    if args:
        if args[0].isdigit():
            if int(args[0]) < 3:
                ...
                if conn:
                    text = tl(update.effective_message, "Diperbarui batas untuk diperingatkan {} pada *{}*").format(
                        args[0], chat_name)
                else:
        ...
        limit, soft_warn, warn_mode = sql.get_warn_setting(chat.id)
        if conn:
            text = tl(update.effective_message, "Batas peringatan saat ini adalah {} pada *{}*").format(limit,
                                                                                                        chat_name)
        else:
...
def set_warn_mode(update, context):
    ...
    if args:
        if args[0].lower() in ("kick", "soft"):
            ...
            if conn:
                text = tl(update.effective_message,
                          "Terlalu banyak peringatan sekarang akan menghasilkan tendangan pada *{}*! Pengguna akan dapat bergabung lagi.").format(
                    chat_name)
            else:
                text = tl(update.effective_message,
            ...
            if conn:
                text = tl(update.effective_message,
                          "Terlalu banyak peringatan akan menghasilkan blokir pada *{}*!").format(chat_name)
            else:
            ...
            if conn:
                text = tl(update.effective_message,
                          "Terlalu banyak peringatan akan menghasilkan bisukan pada *{}*!").format(chat_name)
            else:
        ...
        if not soft_warn:
            if not warn_mode:
                if conn:
                    text = tl(update.effective_message,
                              "Peringatan saat ini disetel ke *tendangan* pengguna saat melampaui batas pada *{}*.").format(
                        chat_name)
                else:
                    text = tl(update.effective_message,
                ...
                if conn:
                    text = tl(update.effective_message,
                              "Peringatan saat ini disetel ke *blokir* pengguna saat melampaui batas pada *{}*.").format(
                        chat_name)
                else:
                    text = tl(update.effective_message,
                ...
                if conn:
                    text = tl(update.effective_message,
                              "Peringatan saat ini disetel ke *bisukan* pengguna saat melampaui batas pada *{}*.").format(
                        chat_name)
                else:
                    text = tl(update.effective_message,
            ...
            if conn:
                text = tl(update.effective_message,
                          "Peringatan saat ini disetel untuk *blokir* pengguna saat melampaui batas pada *{}*.").format(
                    chat_name)
            else:
                text = tl(update.effective_message,
...
def __chat_settings__(chat_id, user_id):
    ...
    return tl(user_id, "Obrolan ini mempunyai `{}` saringan peringatkan. Dibutuhkan `{}` peringatan " \
                       "sebelum pengguna akan mendapatkan *{}*.").format(num_warn_filters, limit,
                                                                         "tendangan" if soft_warn else "pemblokiran")
...
RESET_WARN_HANDLER = CommandHandler(["resetwarn", "resetwarns", "rmwarn"], reset_warns,
...
LIST_WARN_HANDLER = DisableAbleCommandHandler(["warnlist", "warnfilters"],
                                              list_warn_filters)  # , filters=Filters.group, admin_ok=True)
