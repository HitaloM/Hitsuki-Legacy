...
from hitsuki.modules.helper_funcs.extraction import (
    extract_text,
    extract_user,
    ...
    if match:
        ...
        if res:
            update.effective_message.edit_text(
                tld(chat.id, "warns_remove_success").format(
                    mention_html(user.id, user.first_name)
                ),
                parse_mode=ParseMode.HTML,
            )
            ...
            update.effective_message.edit_text(
                tld(chat.id, "warns_user_has_no_warns").format(
                    mention_html(user.id, user.first_name)
                ),
                parse_mode=ParseMode.HTML,
            )
...
def set_warn_strength(bot: Bot, update: Update, args: List[str]):
    ...
    if args:
        ...
        if soft_warn:
            msg.reply_text(
                tld(chat.id, "warns_strength_off"), parse_mode=ParseMode.MARKDOWN
            )
        else:
            msg.reply_text(
                tld(chat.id, "warns_strength_on"), parse_mode=ParseMode.MARKDOWN
            )
    return ""
...
def __stats__():
    return (
        "• <code>{}</code> overall warns, across <code>{}</code> chats.\n"
        "• <code>{}</code> warn filters, across <code>{}</code> chats.".format(
            sql.num_warns(),
            sql.num_warn_chats(),
...
WARN_HANDLER = DisableAbleCommandHandler(
    "warn", warn_user, pass_args=True, filters=Filters.group, admin_ok=True
)
...
ADD_WARN_HANDLER = DisableAbleCommandHandler(
    "addwarn", add_warn_filter, filters=Filters.group, admin_ok=True
)
RM_WARN_HANDLER = DisableAbleCommandHandler(
    ["nowarn", "stopwarn"], remove_warn_filter, filters=Filters.group, admin_ok=True
)
...
WARN_LIMIT_HANDLER = DisableAbleCommandHandler(
    "warnlimit", set_warn_limit, pass_args=True, filters=Filters.group, admin_ok=True
)
