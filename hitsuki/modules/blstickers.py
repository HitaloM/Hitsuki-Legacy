...
from telegram import Message, Chat, ParseMode, User
    ...
    for text in split_text:
        if sticker_list == tl(
            ...
            send_message(
                ...
                tl(
                    update.effective_message,
                    "Tidak ada stiker daftar hitam stiker di <b>{}</b>!").format(chat_name),
...
def add_blackliststicker(update, context):
    ...
    if len(words) > 1:
        ...
        if len(to_blacklist) == 1:
            ...
            send_message(update.effective_message,
                         tl(update.effective_message,
                            "<code>{}</code> stiker ditambahkan ke daftar hitam stiker di <b>{}</b>!").format(added,
                                                                                                              chat_name),
                         parse_mode=ParseMode.HTML)
        ...
        if trigger is None:
            send_message(
                update.effective_message, tl(
                    update.effective_message, "Stiker tidak valid!"))
            return
        try:
            sql.add_to_stickers(chat_id, trigger.lower())
            added += 1
        ...
        send_message(update.effective_message,
                     tl(update.effective_message,
                        "Stiker <code>{}</code> ditambahkan ke daftar hitam stiker di <b>{}</b>!").format(trigger,
                                                                                                          chat_name),
                     parse_mode=ParseMode.HTML)
    else:
    ...
    if len(words) > 1:
        ...
        if len(to_unblacklist) == 1:
            if successful:
                ...
                send_message(
                    update.effective_message, tl(
                        update.effective_message, "Ini tidak ada di daftar hitam stiker...!"))

        elif successful == len(to_unblacklist):
            send_message(update.effective_message,
                         tl(update.effective_message,
                            "Stiker <code>{}</code> dihapus dari daftar hitam di <b>{}</b>!").format(successful,
                                                                                                     chat_name),
                         parse_mode=ParseMode.HTML)
            ...
            send_message(
                update.effective_message, tl(
                    update.effective_message, "Tidak satu pun stiker ini ada, sehingga tidak dapat dihapus.").format(
        ...
        if success:
            send_message(update.effective_message,
                         tl(update.effective_message,
                            "Stiker <code>{}</code> dihapus dari daftar hitam di <b>{}</b>!").format(trigger,
                                                                                                     chat_name),
                         parse_mode=ParseMode.HTML)
        else:
            send_message(update.effective_message,
...
Contoh nilai waktu: 4m = 4 menit, 3h = 3 jam, 6d = 6 hari, 5w = 5 minggu.""")
        ...
        if conn:
            text = tl(
                update.effective_message,
                "Mode blacklist sticker diubah, Pengguna akan `{}` pada *{}*!").format(
                settypeblacklist,
                chat_name)
        else:
            text = tl(
        ...
        if conn:
            text = tl(
                update.effective_message,
                "Mode blacklist sticker saat ini disetel ke *{}* pada *{}*.").format(
                settypeblacklist,
                chat_name)
        else:
            text = tl(
                update.effective_message,
                "Mode blacklist saat ini disetel ke *{}*.").format(settypeblacklist)
            ...
            try:
                if getmode == 0:
                    ...
                    warn(
                        ...
                        tl(
                            update.effective_message,
                            "Menggunakan stiker '{}' yang ada di daftar hitam stiker").format(trigger),
                        message,
                        update.effective_user,
                        conn=False)
                    return
