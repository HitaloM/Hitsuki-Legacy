...
def new_member(update, context):
    ...
    if isAllowed or user.id in SUDO_USERS:
        ...
        msg.reply_text(
            "Thanks for adding me to your group! But this group is not whitelisted to use the bot, sorry.\n\nFollow my news channel. @HitsukiNews")
            ...
            if OWNER_SPECIAL and new_mem.id == OWNER_ID:
                if cleanserv:
                    context.bot.send_message(chat.id, tl(update.effective_message,
                                                         "Master has come home! Let's start this party! 😆"))
                else:
                    send_message(update.effective_message,
                                 tl(update.effective_message, "Master has come home! Let's start this party! 😆"))
                continue
